...
int set_temp_from_ports(temp_t *temp){
    int max_val = 64000;
   	if (THERMO_SENSOR_PORT> max_val){
		...
		short tempt = THERMO_SENSOR_PORT/64 - 500; // ax
		int n = (THERMO_SENSOR_PORT % 64);         //
		if (n >= 32){
			tempt += 1; 
		}
		
		if (!(THERMO_STATUS_PORT & 0b00000001)){  // if 0 then celsius and this masking to check if it's equal to 0
			temp-> is_fahrenheit = 0; //celsius
			temp->tenths_degrees = tempt;
		}
		else{
			temp-> is_fahrenheit = 1; //ferenhite
